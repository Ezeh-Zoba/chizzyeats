...
export const onRatingWrite = onDocumentWritten("ratings/{ratingId}", async (event) => {
    const recipeId = (event.data?.after.data() ?? event.data?.before.data())?.recipeId;
    if (!recipeId)
        return;
    ...
    const average = count === 0
        ? 0
        : Math.round((snapshot.docs.reduce((sum, doc) => sum + doc.data().value, 0) / count) * 10) / 10;
...
export const onNewsletterSubscriberWrite = onDocumentWritten("newsletter_subscribers/{subscriberId}", async (event) => {
    ...
    if (existedBefore === existsAfter)
        return;
    await db.collection("siteConfig").doc("main").set({ subscriberCount: FieldValue.increment(existsAfter ? 1 : -1) }, { merge: true });
});
